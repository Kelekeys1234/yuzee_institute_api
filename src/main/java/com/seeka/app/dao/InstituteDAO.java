...
public class InstituteDAO implements IInstituteDAO {
	...
	public List<InstituteResponseDto> getAllInstitutesByFilter(final CourseSearchDto courseSearchDto, final String sortByField, String sortByType,
		...
		String sqlQuery = "select distinct inst.id as instId,inst.name as instName,inst.city_name as cityName,"
				...
				+ " ,inst.updated_on as updatedOn, it.name as instituteType, inst.campus_type,"
		...
		for (Object[] row : rows) {
			...
			instituteResponseDto.setCampusType(String.valueOf(row[9]));
			instituteResponseDto.setIsActive(Boolean.valueOf(String.valueOf(row[10])));
			if (row[11] != null) {
	...
	public List<InstituteResponseDto> getInstitudeByCityId(final String cityId) {
		...
		String sqlQuery = "select distinct inst.id as instId,inst.name as instName,inst.institute_type_id as institudeTypeId "
				+ "from institute_level instLevel  inner join institute inst " + "on inst.id = instLevel.institute_id " + "where instLevel.city_name ='" + cityId
				+ "' ORDER BY inst.name";
	...
	public List<InstituteResponseDto> getInstituteByListOfCityId(final String citisId) {
		...
		String sqlQuery = "select distinct inst.id as instId,inst.name as instName,inst.institute_type_id as institudeTypeId "
				+ "from institute_level instLevel  inner join institute inst  " + "on inst.id = instLevel.institute_id " + "where instLevel.city_name in ("
...
//	}
	...
	public List<Institute> getAll(final Integer pageNumber, final Integer pageSize) {
		Session session = sessionFactory.getCurrentSession();
		String sqlQuery = "select inst.id, inst.name , inst.country_name , inst.city_name, inst.institute_type_id, inst.description, inst.updated_on , inst.campus_type FROM institute as inst where inst.is_active = 1 and inst.deleted_on IS NULL ORDER BY inst.created_on DESC";
	...
	public List<Institute> autoSearch(final int pageNumber, final Integer pageSize, final String searchKey) {
		Session session = sessionFactory.getCurrentSession();
		String sqlQuery = "select inst.id, inst.name , inst.country_name , inst.city_name, inst.institute_type_id, inst.description, inst.updated_on, inst.campus_type, inst.domestic_ranking FROM institute as inst  "
	...
	public int findTotalCountForInstituteAutosearch(final String searchKey) {
		Session session = sessionFactory.getCurrentSession();
		String sqlQuery = "select inst.id, inst.name , inst.country_name , inst.city_name, inst.institute_type_id, inst.description, inst.updated_on FROM institute as inst  "
				+ " inner join country ctry  on ctry.name = inst.country_name inner join city ci  on ci.id = inst.city_id "
				+ " inner join institute_type instType on instType.id = inst.institute_type_id " + " where inst.deleted_on IS NULL and (inst.name like '%"
				+ searchKey + "%' or inst.description like '%" + searchKey + "%' or ctry.name like '%" + searchKey + "%' or ci.name like '%" + searchKey
	...
	@Override
	public List<NearestInstituteDTO> getNearestInstituteListForAdvanceSearch(AdvanceSearchDto courseSearchDto) {
		Session session = sessionFactory.getCurrentSession();
		String sqlQuery = "SELECT institute.id,institute.name,count(course.id),min(course.usd_international_fee),max(course.usd_international_fee),institute.latitute,institute.longitute," + 
				" 6371 * ACOS(SIN(RADIANS('"+ courseSearchDto.getLatitude() +"')) * SIN(RADIANS(institute.latitute)) +" + 
				" COS(RADIANS('"+ courseSearchDto.getLatitude() +"')) * COS(RADIANS(institute.latitute)) * COS(RADIANS(institute.longitute) -" + 
				" RADIANS('"+ courseSearchDto.getLongitude() +"'))) AS distance_in_km,institute.world_ranking,"+
				" institute.domestic_ranking,avg(course.stars) as stars,course.currency,institute.country_name,institute.city_name,course.name as courseName" +
				//" course.id as courseId,course.cost_range,course.currency,course.duration,course.duration_time,course.language,course.stars,course.recognition, course.domestic_fee, course.international_fee,"+
				//" course.remarks, usd_domestic_fee, usd_international_fee, avg(course.stars) as stars1 "+
				...
				" where institute.latitute is not null and institute.longitute is not null" + 
				" and institute.latitute!= " + courseSearchDto.getLatitude() + " and institute.longitute!= "  + courseSearchDto.getLongitude();
		...
		for (Object[] row : rows) {
			...
			nearestInstituteDTO.setStars(((BigDecimal) row[10]).doubleValue());
			nearestInstituteDTO.setCurrency((String) row[11]);
			nearestInstituteDTO.setCountryName((String) row[12]);
			nearestInstituteDTO.setCityName((String) row[13]);
...
//		}
	...
	@Override
	public List<NearestInstituteDTO> getNearestInstituteList(Integer pageNumber, Integer pageSize, Double latitutde,
			Double longitude, Integer initialRadius) {
		Session session = sessionFactory.getCurrentSession();
		String sqlQuery = "SELECT institute.id,institute.name,count(course.id),min(course.usd_international_fee),max(course.usd_international_fee),institute.latitute,institute.longitute," + 
				" 6371 * ACOS(SIN(RADIANS('"+ latitutde +"')) * SIN(RADIANS(institute.latitute)) +" + 
				" COS(RADIANS('"+ latitutde +"')) * COS(RADIANS(institute.latitute)) * COS(RADIANS(institute.longitute) -" + 
				" RADIANS('"+ longitude +"'))) AS distance_in_km,institute.world_ranking,institute.domestic_ranking,avg(course.stars) as stars,course.currency,institute.country_name,institute.city_name" + 
				...
				" and institute.latitute!= " + latitutde + " and institute.longitute!= "  + longitude + " group by institute.id" + 
		...
		for (Object[] row : rows) {
			...
			nearestInstituteDTO.setWorldRanking((Integer) row[8]);
			nearestInstituteDTO.setDomesticRanking((Integer) row[9]);
			nearestInstituteDTO.setStars((Double) row[10]);
			nearestInstituteDTO.setCurrency((String) row[11]);
			nearestInstituteDTO.setCountryName((String) row[12]);
			nearestInstituteDTO.setCityName((String) row[13]);
			nearestInstituteDTOs.add(nearestInstituteDTO);
