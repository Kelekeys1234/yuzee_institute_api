...
public class InstituteDAO implements IInstituteDAO {
    ...
    /*
     * @Override public Institute getUserByEmail(String email) { Session session = sessionFactory.getCurrentSession(); Criteria crit = session.createCriteria(User.class);
     * crit.add(Restrictions.eq("emailId",email)); List<User> users = crit.list(); return users !=null && !users.isEmpty()?users.get(0):null; }
     */
    ...
    public List<InstituteSearchResultDto> getInstitueBySearchKey(String searchKey) {
        Session session = sessionFactory.getCurrentSession();
        Query query = session
                        .createSQLQuery("select i.id,i.name,c.name as countryName,ci.name as cityName from institute i with(nolock) inner join country c with(nolock) on c.id = i.country_id "
    ...
    public List<InstituteResponseDto> getAllInstitutesByFilter(CourseSearchDto filterObj) {
        ...
        String sqlQuery = "select A.*,count(1) over () totalRows from  (select distinct inst.id as instId,inst.name as instName,ci.name as cityName,"
                        + "ctry.name as countryName,crs.world_ranking,crs.stars,crs.totalCourse " + "from institute inst with(nolock) inner join country ctry with(nolock) "
                        + "on ctry.id = inst.country_id inner join city ci with(nolock) on ci.id = inst.city_id "
                        + "inner join faculty_level f WITH(NOLOCK) on f.institute_id = inst.id " + "inner join institute_level l WITH(NOLOCK) on l.institute_id = inst.id "
                        + "CROSS APPLY ( select count(c.id) as totalCourse, MIN(c.world_ranking) as world_ranking, MIN(c.stars) as stars from " + "course c WITH(NOLOCK) where "
    ...
    public List<InstituteResponseDto> getInstituteByListOfCityId(String citisId) {
        ...
        String sqlQuery = "select distinct inst.id as instId,inst.name as instName,inst.institute_type_id as institudeTypeId "
                        + "from institute_level instLevel with(nolock) inner join institute inst with(nolock) " + "on inst.id = instLevel.institute_id "
                        + "where instLevel.city_id in (" + citisId + ")  ORDER BY inst.name";
        ...
        instituteResponseDto1.setInstituteId(UUID.fromString("CB52B698-98A4-4336-BCC6-98CC1F05EA66"));
