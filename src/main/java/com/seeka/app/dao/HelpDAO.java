...
public class HelpDAO implements IHelpDAO {
	...
	public int findTotalHelpRecord() {
		int status = 1;
		Session session = sessionFactory.getCurrentSession();
		String sqlQuery = "select count(*) from seeka_help sa where sa.is_active = "
				+ status + " and sa.deleted_on IS NULL";
	...
	public List<SeekaHelp> getAll(int pageNumber, Integer pageSize) {
		...
		for (Object[] row : rows) {
			...
			if (row[1] != null) {
				help.setCategory(getHelpCategory(
						new BigInteger(row[1].toString()), session));
			}
			if (row[2] != null) {
				help.setSubCategory(getHelpSubCategory(
						new BigInteger(row[2].toString()), session));
			}
			if (row[3] != null) {
	...
	public BigInteger findTotalHelpRecordBySubCategory(
		...
		String sqlQuery = "select count(*) from seeka_help sa where sa.is_active = "
				+ status
				+ " and sa.deleted_on IS NULL and sub_category_id="
				+ sub_category_id;
		System.out.println(sqlQuery);
		Query query = session.createSQLQuery(sqlQuery);
	...
	public List<HelpSubCategory> getAllHelpSubCategories() {
		Session session = sessionFactory.getCurrentSession();
		List<HelpSubCategory> list = session.createCriteria(
				HelpSubCategory.class).list();
		return list;
	}
