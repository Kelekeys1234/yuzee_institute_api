...
public class ErrorReportController {
    ...
    public ResponseEntity<?> getErrorReportForUser(@RequestHeader(value="userId") BigInteger userId, @PathVariable final Integer pageNumber,
			@PathVariable final Integer pageSize) throws Exception {
    	int startIndex = PaginationUtil.getStartIndex(pageNumber, pageSize);
    	List<ErrorReport> errorReportList =  errorReportService.getAllErrorReportForUser(userId);
    ...
    public ResponseEntity<?> setiIsFavourite(@RequestHeader(value="userId") BigInteger userId, @PathVariable(value="errorReportId") BigInteger errorRepoetId,
    		@PathVariable(value="isFavourite") boolean isFavourite) throws NotFoundException{
    	errorReportService.setIsFavouriteFlag(errorRepoetId, isFavourite);
    	return new GenericResponseHandlers.Builder().setData("acknowledged").setMessage("Updated Successfuly").setStatus(HttpStatus.OK).create();
    }
}
