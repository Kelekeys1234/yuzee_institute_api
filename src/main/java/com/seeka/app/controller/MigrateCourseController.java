...
public class MigrateCourseController {
	...
	public ResponseEntity<?> migrateCityData() throws Exception {
		...
		for (String key : map.keySet()) {
			...
			try {
				...
				coursePricing.setCourseId(course.getId());
	...
	public Map<String, CourseDetails> get() throws Exception{
		File myFile = new File("D:\\SeekaNew\\files\\course\\university_course_attribute_Estonia.xlsx"); 
		...
		for (Faculty faculty : faculties) {
			try {
				facultyMap.put(faculty.getName().toLowerCase().replaceAll("[^\\w]", "")+"-"+faculty.getLevelObj().getId(), faculty);
			}catch(Exception e) {
				e.printStackTrace();
			}
		}
		...
		Map<String, Institute> instituteMap =  new HashMap<String, Institute>();
		for (Institute ins : instituteList) {
			instituteMap.put(ins.getName().toLowerCase().replaceAll("[^\\w]", ""), ins);
		}
		
		
		Map<String, FacultyLevel> facultyLevelMap =  new HashMap<String, FacultyLevel>();
		Map<String, InstituteLevel> instituteLevelMap =  new HashMap<String, InstituteLevel>();
		...
		while (rowIterator.hasNext()) { 
			...
			if(null == instituteLevel) {
				...
				instituteLevel.setCityId(cityObj.getId());
				instituteLevel.setCountryObj(countryObj);
				...
				instituteLevel.setInstituteId(institute.getId());
				instituteLevel.setLevelObj(level);
				//instituteLevelService.save(instituteLevel);
			...
			if(null == facultyLevel) {
				...
				facultyLevel.setFacultyId(facultyObj.getId());
				facultyLevel.setInstituteObj(institute);
				//facultyLevelService.save(facultyLevel);
			...
			course.setCourseLanguage(course_lang);
