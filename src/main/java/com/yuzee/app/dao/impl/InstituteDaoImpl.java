...
public class InstituteDaoImpl implements InstituteDao {
	...
	@Autowired
	private InstituteServiceRepository institueService;
...
//        return obj;
		Query query = new Query();
		if (instituteId != null) {
			query.addCriteria(org.springframework.data.mongodb.core.query.Criteria.where("id").in(instituteId));
		}
		List<InstituteResponseDto> row = mongoTemplate.find(query, InstituteResponseDto.class, "institute");
		InstituteResponseDto ii = new InstituteResponseDto();
		for (InstituteResponseDto r : row) {
			ii.setId(r.getId());
			ii.setLocation(r.getLocation());
			ii.setTotalCount(r.getTotalCount());
			ii.setAboutUs(r.getAboutUs());
			ii.setVisaRequirement(r.getVisaRequirement());
			ii.setTotalAvailableJobs(r.getTotalAvailableJobs());
			ii.setStorageList(r.getStorageList());
			ii.setDistance(r.getDistance());
			ii.setMaxPriceRange(r.getMaxPriceRange());
			ii.setMinPriceRange(r.getMinPriceRange());
			ii.setCurrency(r.getCurrency());
			ii.setInstituteTiming(r.getInstituteTiming());
		}
...
//        return crit.list();
	...
	public void saveInstituteServiceDetails(final InstituteService instituteServiceDetails) {
		institueService.save(instituteServiceDetails);
	}
