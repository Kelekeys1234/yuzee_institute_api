...
//		}
    ...
    private void saveUpdateInstituteFundings(String loggedInUserId, Institute institute,
        ...
        if (!CollectionUtils.isEmpty(instituteFundingDtos)) {
            ...
            fundingsToBeAdded = instituteFundingDtos.stream().filter(dto -> instituteFundings.stream().noneMatch(fromDb -> Objects.equals(fromDb, dto))).collect(Collectors.toList());
            log.info("going to check if funding name ids are valid");
            commonProcessor.getFundingsByFundingNameIds(instituteFundingDtos.isEmpty() ? null : new ArrayList<>(instituteFundingDtos), true);
            instituteFundings.removeIf(fromDb -> instituteFundingDtos.stream().noneMatch(dto -> Objects.equals(fromDb, dto)));
    ...
    private void saveUpdateInstituteProviderCodes(String loggedInUserId, Institute institute,
                                                  List<ProviderCodeDto> providesCodeDtos) throws
            ValidationException, NotFoundException, InvokeException {
   ...
   //     dto.setInstituteYoutubes(getInstituteYoutube(institute.getCountryName(), institute.getName()));
    ...
    @Transactional(rollbackFor = {ConstraintVoilationException.class, Exception.class})
    public InstituteRequestDto getById(final String userId, final String id, final boolean isReadableId) throws
            Exception {
...
//        instituteRequestDto.setFollowed(connectionHandler.checkFollowerExist(userId, instituteRequestDto.getId().toString()));
  ...
  //          String searchQuery = "{ $or : [ {'countryName' : {$regex : ?0, $options : 'i'}, 'cityName' : {$regex : ?0, $options : 'i'}, 'worldRanking' : {$regex : ?0, $options : 'i'}, 'instituteType' : {$regex : ?0, $options : 'i'}, 'name' : {$regex : ?0, $options : 'i'} } ] }";
//        try {
...
//            List<Institute> institutes = instituteRepository.getBySearchText(searchText);
    ...
    public PaginationResponseDto instituteFilter(final InstituteFilterDto instituteFilterDto) {
        ...
        try {
            ...
            if(instituteFilterDto.getPageNumber() != null && instituteFilterDto.getMaxSizePerPage() != null) {
                 ...
                 institutes = instituteDao.instituteFilter(startIndex.intValue(), instituteFilterDto.getMaxSizePerPage(), instituteFilterDto);
            }
    ...
    public int getCountOfInstitute(final CourseSearchDto courseSearchDto, final String searchKeyword,
                                   final String cityId, final String instituteTypeId,
    ...
    @Transactional(rollbackFor = {ConstraintVoilationException.class, Exception.class})
    public List<InstituteResponseDto> getDistinctInstituteList(Integer startIndex, Integer pageSize, String
            instituteName) throws Exception {
...
//        }
    ...
    @Transactional(rollbackFor = {ConstraintVoilationException.class, Exception.class})
    public NearestInstituteDTO getInstitutesUnderBoundRegion(Integer pageNumber, Integer
            pageSize, List<LatLongDto> latLongDtos) throws ValidationException {
    ...
    @Transactional(rollbackFor = {ConstraintVoilationException.class, Exception.class})
    public NearestInstituteDTO getInstituteByCountryName(String countryName, Integer pageNumber, Integer pageSize) throws
            NotFoundException {
    ...
    @Transactional(rollbackFor = {ConstraintVoilationException.class, Exception.class})
    public List<StorageDto> getInstituteGallery(String instituteId) throws
   ...
   //     publishSystemEventHandler.syncInstitutes(Arrays.asList(instituteElasticSearchDto));
    ...
    public void importInstitute(final MultipartFile multipartFile)
            throws
            IOException, ParseException, JobExecutionAlreadyRunningException, JobRestartException, JobInstanceAlreadyCompleteException, JobParametersInvalidException {
    ...
    @Async
    public void exportInstituteToElastic() throws
